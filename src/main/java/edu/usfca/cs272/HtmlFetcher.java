...
public class HtmlFetcher {
	...
	public static boolean isHtml(Map<String, List<String>> headers) {
		...
		return content != null ? content.get(0).startsWith("text/html") : false;
	}
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		...
		try {
			return statusLine != null ? Integer.parseInt(statusLine.get(0).split(" ")[1]) : -1;
		}
	...
	public static String getRedirect(Map<String, List<String>> headers) {
		...
		if (statusCode >= 300 && statusCode <= 399) {
			return redirect != null ? redirect.get(0) != null ? redirect.get(0) : null : null;
		}
		...
		try (
			...
			BufferedReader response = new BufferedReader(input);
				...
				if (isHtml && statusCode == 200) {
					...
					html = output != null ? output : null;
				}
				else if (isHtml && statusCode >= 300 && statusCode <= 399 && redirects > 0) {
